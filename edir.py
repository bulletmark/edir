...
ACTIONS = {
    'remove': (
        ('\033[31m', '\033[30;41m'),  # Red
        ('Removing', 'Remove', 'Removed'),
    ),
    'copy': (
        ('\033[32m', '\033[30;42m'),  # Green
        ('Copying', 'Copy', 'Copied'),
    ),
    'rename': (
...
def log(
    action: str, msg: str, error: str | None = None, *, prompt: bool = False
) -> None:
    ...
    if not args.no_color:
        msg = colors[bool(error and not args.no_invert_color)] + msg + COLOR_reset
...
def remove(
    path: Path, git: bool = False, trash: bool = False, recurse: bool = False
) -> str | None:
    "Delete given file/directory"
    if not recurse and not path.is_symlink() and path.is_dir() and any(path.iterdir()):
...
class Fpath:
        ...
        with fpath.open() as fp:
            for count, line in enumerate(fp, 1):
                ...
                if num <= 0 or num > len(cls.paths):
                    sys.exit(
                        f'ERROR: line {count} number {num} out of range:\n{rawline}'
                    )
    ...
    def get_path_changes(cls) -> list:
        ...
        prompt = (
            create_prompt('proceed/yes edit restart quit[default]')
            if args.interactive
            else None
        )
...
def main() -> int:
    ...
    opt.add_argument(
        ...
        '--interactive',
        action='store_true',
        help='prompt with summary of changes and allow re-edit before proceeding',
    )
    opt.add_argument(
        '-I',
        '--no-interactive',
    ...
    opt.add_argument(
        '-a', '--all', action='store_true', help='include all (including hidden) files'
    )
    opt.add_argument(
        '-A',
        '--no-all',
    ...
    opt.add_argument(
        ...
        '--recurse',
        action='store_true',
        help='recursively remove any files and directories in removed directories',
    )
    opt.add_argument(
        '-R',
        '--no-recurse',
    ...
    opt.add_argument(
        '-t', '--trash', action='store_true', help='use trash program to do deletions'
    )
    opt.add_argument(
        '-T',
        '--no-trash',
    ...
    grp.add_argument('-F', '--files', action='store_true', help='only show/edit files')
    grp.add_argument(
        '-D', '--dirs', action='store_true', help='only show/edit directories'
    )
    opt.add_argument('-L', '--nolinks', action='store_true', help='ignore all symlinks')
    opt.add_argument(
        '-N',
        '--sort-name',
        dest='sort',
